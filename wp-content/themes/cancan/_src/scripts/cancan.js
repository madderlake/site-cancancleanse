...
(function ($) {
  ...
  $('.site-search')
    ...
    $(
      'body:not(.woocommerce-checkout) .site-header-cart, body:not(.woocommerce-cart) .site-header-cart'
    ).click(function () {
      $('.site-header-cart').toggleClass('active');
    }),
    $(
      '.page-template-template-homepage-php .type-page img.wp-post-image'
    ).addClass('loaded'),
    $('.secondary-navigation').length > 0
      ? $('body').addClass('has-secondary-navigation')
      : $('body').addClass('no-secondary-navigation');

  $('ul.wc-tabs li').each(function () {
    var idx = $(this).index();
    var $content = $('.wc-tab').eq(idx);
